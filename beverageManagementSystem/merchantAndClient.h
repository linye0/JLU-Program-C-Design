...
pBeverageNode newBeverageNode(char brand[], char name[], char time[], int storeNum, float price, char info[], pInteractInfo pInfo, int sales); // 根据参数创建一个新的酒水结点
...
pBeverageList sortBeverageSales(pBeverageList List, int key); // 对酒水以销量为关键词进行排序

pBeverageList sortBeverageSaleMoney(pBeverageList List, int key); // 对酒水以销售额为关键词进行排序
